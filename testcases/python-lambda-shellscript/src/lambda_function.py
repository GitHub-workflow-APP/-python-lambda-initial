...
def this_is_another(evt, ctx):
    print("entering this_is_another")
    try:
...
def lambda_handler(event, xyz=333):
    if "queryStringParameters" in event:
        ...
        return indirect_return_bad(event["body"])       # CWEID 80
    elif "what" in event:
        return external_return_bad(event["what"])       # CWEID 80
    elif "how" in event:
        return external_return_good("hi")
    elif "hrmph" in event:
