...
@app.route("/slsep_get/<evt>/<ctx>")
def slsep_get(evt, ctx):
    print("entering slsep_get")
    try:
        if "queryStringParameters" in evt:
            ...
            # we're running in flask
            f = open(evt, "r")              # CWEID 73
            fdata = f.read()
            r = make_response()
            r.set_data(fdata)
            return r
